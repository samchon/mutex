 ...
 * @internal
 */
export class MutexesProvider extends ProviderBase<GlobalMutexes, undefined, void>
{
    /* ---------------------------------------------------------
        WRITE
    --------------------------------------------------------- */
    ...
    {
        return this.get(name).try_lock_for(ms, this.acceptor_, this.createDisolver());
    }
    ...
    {
        return this.get(name).lock_shared(this.acceptor_, this.createDisolver());
    }
    public try_lock_shared(name: string): Promise<boolean>
    {
        return this.get(name).try_lock_shared(this.acceptor_, this.createDisolver());
    }

    public try_lock_shared_for(name: string, ms: number): Promise<boolean>
    {
        return this.get(name).try_lock_shared_for(ms, this.acceptor_, this.createDisolver());
    }
