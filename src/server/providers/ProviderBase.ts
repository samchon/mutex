 ...
 * @internal
 */
export abstract class ProviderBase<GlobalT extends GlobalBase<any, Param, Ret>, Param, Ret>
{
    ...
    /* ---------------------------------------------------------
        CONSTRUCTORS
    --------------------------------------------------------- */
    public constructor(global: GlobalT, acceptor: WebAcceptor<any, any>, disolvers: List<Joiner>, remoteName: string)
    {
        this.global_ = global;
    ...
    public get(name: string): GlobalT extends GlobalBase<infer ComponentT, Param, Ret> ? ComponentT : unknown
    {
        if (this.names_.has(name) === false)
            throw new OutOfRange(`Error on ${this.remote_name_}: you've already erased the component.`);
